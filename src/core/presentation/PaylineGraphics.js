...
export function init(dependencies) { // Renamed initPaylineGraphics
    ...
    // Subscribe to win event - CHANGE to listen for 'paylines:show'
    // _unsubscribeWinEvent = eventBus.on('win:validatedForAnimation', _drawLines); // OLD
    _unsubscribeWinEvent = eventBus.on('paylines:show', _drawLines); // NEW: Listen for specific draw event
...
export function destroy() {
    ...
    if (_unsubscribeWinEvent) {
        ...
        // logger?.debug('PaylineGraphics', 'Unsubscribed from win:validatedForAnimation.'); // OLD
        logger?.debug('PaylineGraphics', 'Unsubscribed from paylines:show.'); // NEW
    }
 ...
 * @param {object} eventData - Payload from the 'paylines:show' event.
...
function _drawLines(eventData) {
    try { // <-- Add try block
        ...
        // Get winningLines directly from eventData
        const winningLines = eventData?.winningLines; 
        if (!winningLines || winningLines.length === 0) {
            currentLogger.debug('PaylineGraphics', 'Received draw event with no winning lines.');
            _clearLines(); 
            return;
        }
        ...
        logger?.error('PaylineGraphics', 'Error in _drawLines handler:', error);
    }
}
...
function _clearLines() { // Renamed clearWinLines and removed export
    try { // <-- Add try block
        ...
        winLineGraphics.alpha = 1;
         ...
         logger?.error('PaylineGraphics', 'Error in _clearLines handler:', error);
    }
}
